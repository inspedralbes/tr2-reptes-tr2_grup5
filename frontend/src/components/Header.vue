<template>
  <template v-if="header.showHeader">
    <aside
      class="nav-bar fixed top-0 left-0 bottom-0 bg-[#022B3A] z-50 flex flex-col shadow-2xl overflow-hidden border-r border-white/5 font-sans"
    >
      <!-- Desktop: barra lateral vertical (amagada a ≤692px) -->
      <div class="nav-bar-desktop flex flex-col flex-1 min-h-0">
        <header class="p-8 pb-6 flex flex-col items-start">
          <div class="logo-container w-full max-w-[210px]">
      ...
      <!-- Mòbil (≤692px): barra horitzontal amb logo i hamburger -->
      <div class="nav-bar-mobile">
        <div class="mobile-logo-wrap">
          <img src="/logo_v_white.png" alt="Logo Enginy" class="h-9 w-auto object-contain" />
        </div>
        <button
          type="button"
          @click="menuOpen = !menuOpen"
          class="p-2.5 rounded-lg text-white/80 hover:text-white hover:bg-white/10 transition-all"
    ...
    <!-- Dropdown mòbil: tots els botons (incl. Sortir) -->
    <div
      v-if="menuOpen"
    ...
    >
      <div class="mobile-dropdown-inner">
        <button
          v-for="(btn, i) in allMenuItemsForMobile"
          :key="'mob-' + (btn.route || '') + '-' + i"
          type="button"
          @click="navegar(btn); menuOpen = false"
          :class="[
            'w-full flex items-center gap-4 px-5 py-4 rounded-xl transition-all text-left',
            isActive(btn)
              ? 'bg-gradient-to-r from-[#1F7A8C] to-[#1A6B7A] text-white'
        ...
        >
          <component
            ...
            :strokeWidth="isActive(btn) ? 2.5 : 1.5"
          />
          <span :class="['text-sm', isActive(btn) ? 'font-black' : 'font-medium']">{{ btn.label }}</span>
        </button>
      </div>
    </div>
  </template>
